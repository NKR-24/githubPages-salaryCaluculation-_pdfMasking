...
export const REDACTOR_VERSION = "2.2";
...
const nameLikeToken = (t) => t.replace(GENERIC_CHARS, "").length >= 2;

const rectContains = (rect, cx, cy) =>
  cx > rect[0] + 0.25 && cx < rect[2] - 0.25 && cy > rect[1] + 0.25 && cy < rect[3] - 0.25;
 ...
 */
export async function verifyDocumentAsync(mupdf, outData, maskResult, onProgress) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    const rects = (maskResult.pagePlans[i] || { rects: [] }).rects;
    for (const line of lines) {
      for (const ch of line.chars) {
        ...
        for (const item of rects) {
          if (rectContains(item.rect, cx, cy)) {
  ...
  const tokenLeaks = [];
  let n = 0;
  for (let i = 0; i < maskResult.pagePlans.length; i++) {
    for (const item of maskResult.pagePlans[i].rects) {
      const tok = item.token || "";
      if (tok.length >= 2 && nameLikeToken(tok) && allTextNoSpace.includes(tok)) {
